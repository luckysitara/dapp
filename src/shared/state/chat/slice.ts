...
export interface EncryptedMessagePayload {
  ...
  ciphertext: string;
  tag: string;
  senderPublicKey: string; // Public key of the sender (E25519 or Solana PubKey)
  chatId: string; // The chat room ID
  messageId: string; // The ID of the encrypted message on the server
  timestamp: number;
}
...
export interface ChatMessage {
  ...
  updated_at: string;
  is_deleted?: boolean;
  is_encrypted?: boolean; // Flag to indicate if it was an E2EE message
  sender?: {
    id: string;
    username: string;
    profile_picture_url: string | null;
...
// Async Thunks
export const processEncryptedMessage = createAsyncThunk(
  'chat/processEncryptedMessage',
  async (encryptedPayload: EncryptedMessagePayload, { dispatch, rejectWithValue, getState }) => {
    try {
      const state: any = getState(); // Access the entire Redux state
      const currentSeekerPublicKeyBase58 = state.auth.address;

      if (!currentSeekerPublicKeyBase58) {
        throw new Error('Current user public key not found in Redux state.');
      }

      const seekerX25519SecretKeyBase64 = await SecureStore.getItemAsync(`e2ee-keypair-${currentSeekerPublicKeyBase58}`);
      if (!seekerX25519SecretKeyBase64) {
        throw new Error('Seeker X25519 secret key not found in SecureStore.');
      }
      const seekerX25519SecretKey = Buffer.from(seekerX25519SecretKeyBase64, 'base64');

      const senderX25519PublicKey = await EncryptionService.fetchRecipientX25519PublicKey(encryptedPayload.senderPublicKey);
      ...
      const decryptedContent = await EncryptionService.decrypt(sharedSecret, encryptedPayload);

      const decryptedMessage: ChatMessage = {
        id: encryptedPayload.messageId,
        chat_room_id: encryptedPayload.chatId,
        sender_id: encryptedPayload.senderPublicKey,
        content: decryptedContent,
        created_at: new Date(encryptedPayload.timestamp).toISOString(),
        updated_at: new Date(encryptedPayload.timestamp).toISOString(),
        is_encrypted: true, // Mark as originally encrypted
        // Other fields like sender username/profile pic would be fetched or derived
      };
      
      dispatch(receiveMessage(decryptedMessage)); // Dispatch the decrypted message
