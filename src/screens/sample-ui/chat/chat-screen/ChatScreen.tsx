...
function ChatScreen(): React.ReactElement {
  ...
  const { address, publicKey, signMessage, sendTransaction } = useWallet();
  const auth = useAppSelector(state => state.auth);

  // Store user's ephemeral X25519 key pair
  ...
  useEffect(() => {
    const loadOrCreateKeyPair = async () => {
      ...
      const keyPairString = await AsyncStorage.getItem(`x25519_key_pair_${address}`);
      if (keyPairString) {
        const storedKeyPair = JSON.parse(keyPairString);
        setEncryptionKeyPair({
            publicKey: util.decodeBase64(storedKeyPair.publicKey),
            secretKey: util.decodeBase64(storedKeyPair.secretKey),
        });
      } else {
        const newKeyPair = EncryptionService.generateKeyPair();
        await AsyncStorage.setItem(`x25519_key_pair_${address}`, JSON.stringify({
            publicKey: util.encodeBase64(newKeyPair.publicKey),
            secretKey: util.encodeBase64(newKeyPair.secretKey),
        }));
        setEncryptionKeyPair(newKeyPair);
      }
    };
  ...
  const isOneOnOneChat = useMemo(() => {
    if (isGroup || chatId === 'global' || chatId === AI_AGENT.id) {
        return false;
    }
    try {
        const decodedChatId = util.decodeBase64(chatId);
        // Check if the decoded chatId is a valid Ed25519 public key (32 bytes)
        // Also ensure it can be converted to an X25519 key for shared secret derivation
        return decodedChatId.length === web3.nacl.publicKeyLength;
    } catch (e) {
        // If decoding or PublicKey creation fails, it's not a valid public key
        console.warn('Chat ID is not a valid base64 encoded Ed25519 public key:', e);
        return false;
    }
  }, [isGroup, chatId, AI_AGENT.id]);
