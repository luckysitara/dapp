...
const ChatListScreen: React.FC = () => {
  ...
  const [resolvingNames, setResolvingNames] = useState<{ [key: string]: boolean }>({}); // New state for name resolution
  ...
  // State to store chats with resolved names for display
  const [displayChats, setDisplayChats] = useState<ChatRoom[]>([]);
  ...
  useEffect(() => {
    const resolveNamesForChats = async () => {
      const resolvedChats = await Promise.all(
        sortedChats.map(async chat => {
          if (chat.type === 'direct' && chat.participants) {
            ...
            if (otherParticipant) {
              // Check if name is already resolving or resolved
              if (resolvingNames[otherParticipant.id]) {
                return chat; // Skip if already resolving
              }

              // Set resolving status
              setResolvingNames(prev => ({ ...prev, [otherParticipant.id]: true }));

              try {
                const name = await findDisplayName(connection, new PublicKey(otherParticipant.id));
                const resolvedName = name?.name || otherParticipant.username;
                return {
                  ...chat,
                  resolvedParticipantName: resolvedName,
                  resolvedParticipantHandle: name?.name, // Store actual .sol/.skr handle if found
                };
              } catch (error) {
                console.error(`Failed to resolve name for ${otherParticipant.id}:`, error);
                return { ...chat, resolvedParticipantName: otherParticipant.username }; // Fallback
              } finally {
                setResolvingNames(prev => ({ ...prev, [otherParticipant.id]: false }));
              }
            }
          }
          return chat;
      ...
      setDisplayChats(resolvedChats);
    };

    if (sortedChats.length > 0) {
      resolveNamesForChats();
    } else {
      setDisplayChats([]);
    }
  }, [sortedChats, connection, userId, resolvingNames]);


  // Resolve participant names
  useEffect(() => {
    const resolveNames = async () => {
      const updatedDisplayChats = await Promise.all(
        filteredChats.map(async chat => {
          if (chat.type === 'direct' && chat.participants) {
            const otherParticipant = chat.participants.find(p => p.id !== userId);
            if (otherParticipant && !resolvingNames[otherParticipant.id]) {
              setResolvingNames(prev => ({ ...prev, [otherParticipant.id]: true }));
              try {
                const name = await findDisplayName(connection, new PublicKey(otherParticipant.id));
                const resolvedName = name?.name || otherParticipant.username;
                setResolvingNames(prev => ({ ...prev, [otherParticipant.id]: false }));
                return { ...chat, resolvedParticipantName: resolvedName };
              } catch (error) {
                console.error(`Failed to resolve name for ${otherParticipant.id}:`, error);
                setResolvingNames(prev => ({ ...prev, [otherParticipant.id]: false }));
                return { ...chat, resolvedParticipantName: otherParticipant.username };
              }
            }
          }
          return chat;
      ...
      setDisplayChats(updatedDisplayChats);
    };
  ...
  }, [filteredChats, connection, resolvingNames, userId]);
  ...
  const prepareChats = useCallback(() => {
    ...
    */
    ...
    const apiChats = displayChats.map(chat => {
  ...
  const handleChatPress = useCallback((chat: any) => {
    ...
    if (chat.type === 'direct' && chat.participants) {
      ...
      if (otherParticipant) {
        recipientPublicKey = otherParticipant.id;
      }
    }
  ...
  }, [navigation, handleAIAgentChat, dispatch, userId]);
    ...
    if (isDirect && item.participants) {
              ...
              if (otherParticipant) {
                ...
                participantUser = {
                  ...
                  username: item.resolvedParticipantName || otherParticipant.username, // Use resolved name
                  handle: item.resolvedParticipantHandle || otherParticipant.username, // Use resolved handle
